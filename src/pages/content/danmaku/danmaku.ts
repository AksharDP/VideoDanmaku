...
export class Danmaku {
	...
	*/
	...
	private calculateLanes(): void {
		...
		this.debugLog(`Container height: ${screenHeight}, lane height: ${this.laneHeight}`);
	...
	private addVideoEventListeners(): void {
		...
		this.debugLog("Added container resize observer.");
	}
	...
	private positionHoverPopup(popup: HTMLElement, event: MouseEvent, target: HTMLElement): void {
		...
		const top = commentRect.bottom - containerRect.top
		let popupTop = top;
		if (popupTop + popupHeight > containerRect.height) {
			popupTop = commentRect.top - containerRect.top - popupHeight
		} else if (popupTop < 0) {
			popupTop = 0;
		}
